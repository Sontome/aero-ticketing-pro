...
export const fetchVietJetFlights = async (searchData: SearchFormData): Promise<Flight[]> => {
  ...
  try {
    const response = await fetch('https://completion-football-cancelled-moscow.trycloudflare.com/vj/check-ve-v2', {
      method: 'POST',
      headers: {
    ...
    return data.body.map((flight, index) => ({
      id: `vj-${flight['chiều đi'].id}-${index}`,
      airline: 'VJ' as const,
      flightNumber: `VJ${flight['chiều đi'].id}`,
      departure: {
...
export const fetchVietnamAirlinesFlights = async (searchData: SearchFormData): Promise<Flight[]> => {
  ...
  try {
    const response = await fetch('https://completion-football-cancelled-moscow.trycloudflare.com/vna/check-ve-v2', {
      method: 'POST',
      headers: {
        'accept': 'application/json',
        'Content-Type': 'application/json',
